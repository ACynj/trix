# 快速提升普适性 - 实施指南

## 🎯 核心问题
当前方法在部分数据集上表现好（WN18RR、HM），但在大规模数据集上下降（FB15k237、ILPC2022）。

## ⚡ 最快见效的方案：调整Beta值

### 方案1：手动测试不同Beta（推荐先做）

在表现差的数据集上测试不同beta值：

```bash
# FB15k237 v1 - 测试不同beta
python ./src/run_relation.py \
  -c ./config/run_relation_inductive_mech.yaml \
  --dataset FB15k237Inductive --version v1 \
  --ckpt /T20030104/ynj/TRIX/ckpts/rel_5.pth \
  --gpus [0] --epochs 0 --bpe null \
  --beta 1.0e-4  # 尝试：1e-4, 5e-4, 1e-3, 2e-3
```

**预期结果：**
- beta=1e-4 或 5e-4 可能在FB15k237上恢复性能
- 找到最优beta后，更新配置文件

### 方案2：创建数据集特定配置

为表现差的数据集创建专门配置：

```bash
# 复制当前配置
cp config/run_relation_inductive_mech.yaml config/run_relation_inductive_mech_fb15k237.yaml

# 修改beta值
# 在配置文件中将 beta: 1.0e-3 改为 beta: 1.0e-4
```

### 方案3：根据数据集规模自动选择Beta

在训练脚本中添加逻辑：

```python
# 伪代码，需要添加到run_relation.py
num_train_samples = len(train_data.target_edge_index[0])
if num_train_samples < 10000:
    beta = 1e-3
elif num_train_samples < 50000:
    beta = 5e-4
else:
    beta = 1e-4

# 更新配置
cfg.model.mechanism.beta = beta
```

## 📊 建议的实验顺序

### 第一步：快速验证（1-2小时）
在FB15k237 v1上测试4个beta值：
- beta = 1e-4
- beta = 5e-4  
- beta = 1e-3（当前）
- beta = 2e-3

**目标：** 找到能让FB15k237性能恢复或超过baseline的beta

### 第二步：扩展到其他数据集（半天）
对以下数据集分别找最优beta：
- FB15k237 v2-v4
- ILPC2022 small/large
- FBIngram 25/50/75/100

**目标：** 建立"数据集规模 → beta"的映射关系

### 第三步：验证改进（1天）
用找到的beta值重新跑所有数据集，对比：
- 之前表现好的数据集是否仍然好
- 之前表现差的数据集是否改善

## 🔧 具体操作步骤

### 1. 修改配置文件支持beta参数

如果配置文件不支持命令行传beta，需要：

**选项A：** 直接修改配置文件中的beta值
```yaml
# config/run_relation_inductive_mech.yaml
mechanism:
  beta: 1.0e-4  # 改为测试值
```

**选项B：** 使用模板变量（如果支持）
```yaml
# config/run_relation_inductive_mech_adaptive.yaml
mechanism:
  beta: {{ beta | default(1.0e-3) }}
```

### 2. 批量测试脚本

创建 `test_beta.sh`:
```bash
#!/bin/bash
for beta in 1.0e-4 5.0e-4 1.0e-3 2.0e-3; do
  echo "Testing beta=$beta"
  # 修改配置文件中的beta值
  sed -i "s/beta: .*/beta: $beta/" config/run_relation_inductive_mech.yaml
  # 运行测试
  python ./src/run_relation.py \
    -c ./config/run_relation_inductive_mech.yaml \
    --dataset FB15k237Inductive --version v1 \
    --ckpt /T20030104/ynj/TRIX/ckpts/rel_5.pth \
    --gpus [0] --epochs 0 --bpe null
done
```

### 3. 记录结果

创建结果表格，记录每个beta的性能：

| Beta | FB15k237 v1 MRR | FB15k237 v1 Hits@1 | ILPC2022 small MRR | ... |
|------|----------------|-------------------|-------------------|-----|
| 1e-4 | ? | ? | ? | ... |
| 5e-4 | ? | ? | ? | ... |
| 1e-3 | 0.663 | 0.518 | 0.905 | ... |
| 2e-3 | ? | ? | ? | ... |

## 🎯 预期改进

### 短期目标（1-2天）
- FB15k237系列：MRR恢复到0.68+（接近baseline 0.69）
- ILPC2022：MRR恢复到0.91+（接近baseline 0.92）

### 中期目标（1周）
- 所有数据集性能 ≥ baseline
- 建立自适应beta规则

### 长期目标（2周+）
- 实现自动beta选择
- 在所有数据集上都有提升

## ⚠️ 注意事项

1. **保持其他超参不变**：只改beta，确保对比公平
2. **使用相同checkpoint**：都用rel_5.pth
3. **记录完整结果**：保存所有指标，便于分析
4. **多次运行**：如果时间允许，每个beta跑3次取平均

## 📝 下一步

完成beta调优后，如果还有数据集表现差，考虑：
1. 调整z_dim（16/32/64）
2. 改进机制编码器（增加层数）
3. 使用KL warmup策略

详见 `improve_generality.md` 中的详细方案。

